...
void A1_Motor_Multiple_Control(int ID,int mode,float Torque, float W,float Position)
{
	...
	Motor_Tx_u.Tx_Message.W = (int16_t)(W * 128.0f * 9.1f);
	Motor_Tx_u.Tx_Message.Pos = (int32_t)(Position * (16384.0f / (2.0f * PI)) * 9.1f);
	Motor_Tx_u.Tx_Message.kp = (uint16_t)(0*2048.0f);
...
void A1_Motor_Speed_Position_Control(int ID,float W,float Position)
{
	if (fabs((double)((A1_State[ID].Position)-Position)) < 0.1f) return;
	float deadbond = 0.06f*W;
...
void Received_Data_Dealer(const uint8_t *sbus_buf)
{
	...
	Motor_Rx_u.data[12] = sbus_buf[34];
	Motor_Rx_u.data[13] = sbus_buf[35];
	Motor_Rx_u.data[14] = sbus_buf[36];
	Motor_Rx_u.data[15] = sbus_buf[37];
	A1_State[ID].Mode = Motor_Rx_u.Received_data.mode;
	A1_State[ID].Temp = Motor_Rx_u.Received_data.Temp;
	A1_State[ID].Torque = (((float)Motor_Rx_u.Received_data.T)/256.0f);
	A1_State[ID].Omega = ((float)Motor_Rx_u.Received_data.W)/(128.0f*9.1f);
	A1_State[ID].Acc = Motor_Rx_u.Received_data.Acc;
	A1_State[ID].Position = ((float)Motor_Rx_u.Received_data.Pos*2.0f*PI)/(16384.0f*9.1f);
	A1_State[ID].Position2 = Motor_Rx_u.Received_data.Pos2;
}
